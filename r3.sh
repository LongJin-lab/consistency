...
dir_loc='landscape'
device='9'
train_dir='train_ori.py' #train_ori1.py train_ori_gradient.py train_ori_gradient_3step.py
        ...
        for ((k=1;k<2;k++));do #b1,b2,b3系数
            for ((i=0;i<1;i++));do #模型
                for ((j=0;j<3;j++));do #重复实验次数
                    #⬇1step⬇
                    # CUDA_VISIBLE_DEVICES=${device} nohup /home/lab416/anaconda3/envs/pt37_backup/bin/python ${train_dir} --repnum ${j} --nnlayer ${i} --tao ${k} --epoch ${epoch} -bs ${bs} --lr ${lr} --h ${tao[device]} --a0 ${a0[l]} --b1 ${b1[k]} --arch ${model[i]} > ./log/${dir_loc}/tao_${tao[device]}_a0_${a0[l]}_b1_${b1[k]}_${model[i]}_${j}.txt 2>&1
                    #⬇2step⬇
