...
table_loc='log/gradient/'+'h-'+str(args.h)+'-[b1,b2,b3]-['+str(args.b1)+','+str(args.b2)+','+str(args.b3)+']-'+str(args.arch)+'_'+str(args.repnum)+'.xlsx'
...
ws = wb.active
# 在第一行的第一列和第二列写入数据
ws.cell(row=1, column=1).value = 'norm_2'
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        global ite_num,table_loc,fig_num
        if ite_num%30==0:
            grads = []
            for param in net.parameters():
                try:
                    tmp_list=param.grad.flatten().tolist()
                    grads = grads+tmp_list
                except AttributeError:
                    continue
                    # print("x is None and cannot be flattened")
                # if param is None:
                #     continue
                # tmp_list=param.grad.flatten().tolist()
                # grads = grads+tmp_list
            ...
            sheet.cell(row=fig_num, column=1).value = np.linalg.norm(np.array(grads))
            # sheet.cell(row=ite_num, column=1).value = max(list(map(abs, grads)))
            # sheet.cell(row=ite_num, column=2).value = sum(map(abs, grads)) / len(grads)
            # print(len(grads))
            wb.save(table_loc)
...
#            train_scheduler.step(epoch)
    ...
    # colqc=args.repnum+3
    # from openpyxl import load_workbook
    # wb = load_workbook(r'log/12.19/12.19.xlsx')
    # sheet = wb.active
    # sheet.cell(row=rowqc, column=colqc).value = test_top1_acc.item()
    # coltem=colqc+7
    # sheet.cell(row=rowqc, column=coltem).value = train_top1_acc.item()
    # wb.save(r'log/12.19/12.19.xlsx')
    # print("运行结束！")
    # print("0.3-4.5")
