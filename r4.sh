...
dir_loc='12.22'
device='9'
train_dir='train_100ori.py' #train_ori1.py

for ((h=0;h<10;h++));do #tao 步长
    for ((l=0;l<1;l++));do #a0系数
        for ((k=0;k<2;k++));do #b1,b2,b3系数
            for ((i=4;i<5;i++));do #模型
                for ((j=2;j<3;j++));do #重复实验次数
                    #⬇1step⬇
                    CUDA_VISIBLE_DEVICES=${device} nohup /home/lab416/anaconda3/envs/pt37_backup/bin/python ${train_dir} --repnum ${j} --nnlayer ${i} --tao ${k} --epoch ${epoch} -bs ${bs} --lr ${lr} --h ${tao[h]} --a0 ${a0[l]} --b1 ${b1[k]} --arch ${model[i]} > ./log/${dir_loc}/tao_${tao[h]}_a0_${a0[l]}_b1_${b1[k]}_${model[i]}_${j}.txt 2>&1
                    #⬇2step⬇
                    # CUDA_VISIBLE_DEVICES=${device} nohup /home/lab416/anaconda3/envs/pt37_backup/bin/python ${train_dir} --repnum ${j} --nnlayer ${i} --tao ${k} --epoch ${epoch} -bs ${bs} --lr ${lr} --h ${tao[h]} --a0 ${a0[l]} --b1 ${b1[k]} --b2 ${b2[k]} --arch ${model[i]} > ./log/${dir_loc}/tao_${tao[h]}_a0_${a0[l]}_b1_${b1[k]}_b2_${b2[k]}_${model[i]}_${j}.txt 2>&1
                    #⬇3step⬇
                    # CUDA_VISIBLE_DEVICES=${device} nohup /home/lab416/anaconda3/envs/pt37_backup/bin/python ${train_dir} --repnum ${j} --nnlayer ${i} --tao ${k} --epoch ${epoch} -bs ${bs} --lr ${lr} --h ${tao[h]} --a0 ${a0[l]} --b1 ${b1[k]} --b2 ${b2[k]} --b3 ${b3[k]} --arch ${model[i]} > ./log/${dir_loc}/tao_${tao[h]}_a0_${a0[l]}_b1_${b1[k]}_b2_${b2[k]}_b3_${b3[k]}_${model[i]}_${j}.txt 2>&1
                done;
            done;
        done;
    done;
